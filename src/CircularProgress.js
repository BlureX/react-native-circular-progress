...
export default class CircularProgress extends React.PureComponent {
	...
	render() {
		...
		const backgroundPath = this.circlePath(size / 2, size / 2, size / 2 - maxWidthCircle / 2, 0, arcSweepAngle);
			...
			<View style={style}>
				<Svg width={size} height={size} style={{ backgroundColor: "transparent" }}>
					<Svg.G rotation={rotation} originX={size / 2} originY={size / 2}>
						...
						{fill > 0 && (
							<Svg.Path d={circlePath} stroke={tintColor} strokeWidth={width} strokeLinecap={lineCap} fill="transparent" />
						)}
					</Svg.G>
				</Svg>
